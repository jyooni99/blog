...
아래는 `useState`을 사용할 때와 `React Hook Form`을 사용할 때의 리렌더링 차이를 보여주는 영상입니다.

`useState`를 사용할 때:

<video autoPlay loop muted playsInline preload="metadata">
  <source src="/posts/qn-usestate-to-rhf/usestate.mp4" type="video/mp4" />
</video>

`React Hook Form`을 사용할 때:

<video autoPlay loop muted playsInline preload="metadata">
  <source src="/posts/qn-usestate-to-rhf/rhf.mp4" type="video/mp4" />
</video>
